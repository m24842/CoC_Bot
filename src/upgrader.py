...
class Upgrader:
    ...
    def lab_available(self, timeout=60):
        ...
        while time.time() < start + timeout:
            try:
                ...
                if configs.DEBUG: Frame_Handler.save_frame(section, "debug/lab.png")
    ...
    def upgrade(self):
        try:
            ...
            if not check_color([255, 136, 127], section, tol=10):
                ...
                section = Frame_Handler.get_frame_section(x-0.08, y+0.02, x+0.08, y+0.1, high_contrast=True)
                if configs.DEBUG: Frame_Handler.save_frame(section, "debug/upgrade_cost.png")
                # resource_type = self.get_resource_type(Frame_Handler.get_frame_section(x-0.08, y+0.02, x+0.08, y+0.1, grayscale=False))
    ...
    def run(self):
        ...
        while counter < MAX_UPGRADES_PER_CHECK:
            counter += 1
            try:
                initial_builders = get_builders(1)
                if initial_builders == 0: break
                upgraded = self.upgrade()
                Input_Handler.click_exit(5, 0.1)
                time.sleep(0.5)
                final_builders = get_builders(1)
